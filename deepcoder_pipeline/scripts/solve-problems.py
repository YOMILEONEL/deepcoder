...
def solve_problems(problems, T, mode='dfs', gas=np.inf):
    ...
    # Windows spawns each worker fresh (no fork), so every worker re-imports
    # this module's top-level deepcoder.nn.model -> keras/tensorflow/jax
    # chain from scratch. With os.cpu_count() workers doing that at once,
    # peak memory during startup can exceed what's available and crash the
    # pool (BrokenProcessPool from a worker MemoryError). Capping workers
